 ...
 * LangBehavior class
 *
 * @property \yii\db\ActiveQuery $langsRef
 * @property \yii\db\ActiveQuery $langsRefCurrent
...
class LangBehavior extends \yii\base\Behavior
{
	...
	public $langModelClass = 'common\models\Lang'; // класс модели языков

	/* @var ActiveRecord[] $_langs */
	private $_langs = null;
	/* @var ActiveRecord $_lang */
	private $_lang = null;
	...
	{
		/* @var \common\models\Lang $langModel */
	...
	{
		if ($this->_langs === null) {
			/* @var ActiveRecord $langModel */
			$langModel = $this->langModelClass;
			/* @var ActiveRecord[] $all_langs */
	...
	public function getFieldItemsTranslatedList($field, $lang=null)
	{
		...
		if ($lang === null) {
			...
			$items->where([$langModel::tableName() . '.id' => $lang->id]);
		}
