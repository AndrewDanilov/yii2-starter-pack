...
<div class="shop-category-form">
	...
	<?= $form->field($model, 'parent_id')->dropDownList(NestedCategoryHelper::getDropdownTree(), ['prompt' => 'Верхний уровень', 'style' => 'font-family:monospace;']) ?>
	...
	<?= $form->field($model, 'image')->widget(InputFile::class, [
		...
		'template'      => '<div>' . Html::img($model->image, ['width' => '300', 'class' => 'preview']) . '</div><div class="input-group">{input}<span class="input-group-btn">{button}</span></div>',
		'options'       => ['class' => 'form-control'],
		'buttonOptions' => ['class' => 'btn btn-default'],
		'multiple'      => false,      // возможность выбора нескольких файлов
	]) ?>
	<?php $this->registerJs("$('#shopcategory-image').on('change', function() { $(this).parents('.form-group').find('img.preview').attr('src', $(this).val()); });"); ?>
